# Book-to-Repository Publishing Policy

## Publish publicly

Good candidates for the public repository:

- original TypeScript source code
- exercise starter files
- reference solutions you are comfortable open-sourcing
- automated tests
- sample configuration files with fake values
- small sample datasets created for the project
- diagrams created specifically for the repository
- companion apps and reusable libraries
- issue templates and contribution documentation

## Keep outside the public repository

Unless you intentionally want to give them away under a public license, do not commit:

- the complete paid PDF or DOCX book
- full chapter manuscripts
- print-ready cover/source files
- store-upload packages meant for commercial distribution
- private sales/account documents
- unpublished drafts
- API keys, credentials, or private user data

## Recommended licensing split

Use an open-source license such as MIT for code you want others to reuse. Keep the book/editorial content under a separate copyright notice. A repository can contain both notices as long as it is clear which files each notice covers.

## Versioning recommendation

Tag major companion-code milestones around stable book releases, for example `v1.0.0` for the first complete 120-part companion structure. Use normal semantic versioning for code changes rather than matching every typo correction in the book.
